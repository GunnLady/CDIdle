...
test("loads all twenty CDI-149 Warrior combat-idle sprites across the five PC cinema review pages", async ({ page }) => {
  ...
  for (let reviewPage = 1; reviewPage <= 5; reviewPage += 1) {
    ...
    const expectedNames = (["female", "male"] as const).flatMap((gender) => (
      [firstVariant, firstVariant + 1].map((variant) => (
        `warrior-${gender}-${String(variant).padStart(2, "0")}-combat-idle-v1`
      ))
    ));
...
test("loads all twenty CDI-150 Rogue combat-idle sprites across the five PC cinema review pages", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 });
  ...
  for (let reviewPage = 1; reviewPage <= 5; reviewPage += 1) {
    ...
    const expectedNames = (["female", "male"] as const).flatMap((gender) => (
      [firstVariant, firstVariant + 1].map((variant) => (
        `rogue-${gender}-${String(variant).padStart(2, "0")}-combat-idle-v1`
      ))
    ));
    expect(sources).toHaveLength(4);
    expect(sources.every(({ naturalWidth, naturalHeight }) => (
      naturalWidth >= 366 && naturalWidth <= 1042 && naturalHeight === 920
    ))).toBe(true);
    for (const expectedName of expectedNames) {
      expect(sources.some(({ source }) => source.includes(expectedName))).toBe(true);
    }
...
test("loads all twenty CDI-153 Acolyte combat poses across the five PC cinema review pages", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 });

  for (let reviewPage = 1; reviewPage <= 5; reviewPage += 1) {
    await page.goto(`/tests/browser/fixtures/dungeon-harness.html?acolyte-cinema=${reviewPage}`);
    const scene = page.getByTestId("dungeon-combat-scene");
    const heroes = scene.locator("[data-testid='dungeon-combat-actor'][data-team='heroes']");
    await expect(scene).toBeVisible();
    await expect(heroes).toHaveCount(4);
    ...
    const expectedNames = (["female", "male"] as const).flatMap((gender) => (
      [firstVariant, firstVariant + 1].map((variant) => (
        `acolyte-${gender}-${String(variant).padStart(2, "0")}-combat-idle-v1`
      ))
    ));
    expect(sources).toHaveLength(4);
    expect(sources.every(({ naturalWidth, naturalHeight }) => (
...
test("loads all twenty CDI-142 Aede sprites across the five PC cinema review pages", async ({ page }) => {
  ...
  for (let reviewPage = 1; reviewPage <= 5; reviewPage += 1) {
    await page.goto(`/tests/browser/fixtures/dungeon-harness.html?aede-cinema=${reviewPage}`);
    const scene = page.getByTestId("dungeon-combat-scene");
    const heroes = scene.locator("[data-testid='dungeon-combat-actor'][data-team='heroes']");
    await expect(scene).toBeVisible();
    await expect(heroes).toHaveCount(4);
...
test("loads all twenty CDI-143 Druid sprites across the five PC cinema review pages", async ({ page }) => {
  ...
  for (let reviewPage = 1; reviewPage <= 5; reviewPage += 1) {
    await page.goto(`/tests/browser/fixtures/dungeon-harness.html?druid-cinema=${reviewPage}`);
    const scene = page.getByTestId("dungeon-combat-scene");
    const heroes = scene.locator("[data-testid='dungeon-combat-actor'][data-team='heroes']");
    await expect(scene).toBeVisible();
    await expect(heroes).toHaveCount(4);
...
test("loads all twenty CDI-144 Artificer sprites across the five PC cinema review pages", async ({ page }) => {
  ...
  for (let reviewPage = 1; reviewPage <= 5; reviewPage += 1) {
    await page.goto(`/tests/browser/fixtures/dungeon-harness.html?artificer-cinema=${reviewPage}`);
    const scene = page.getByTestId("dungeon-combat-scene");
    const heroes = scene.locator("[data-testid='dungeon-combat-actor'][data-team='heroes']");
    await expect(scene).toBeVisible();
...
test("loads all twenty CDI-145 Pugilist sprites across the five PC cinema review pages", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 });

  for (let reviewPage = 1; reviewPage <= 5; reviewPage += 1) {
    await page.goto(`/tests/browser/fixtures/dungeon-harness.html?pugilist-cinema=${reviewPage}`);
    const scene = page.getByTestId("dungeon-combat-scene");
    const heroes = scene.locator("[data-testid='dungeon-combat-actor'][data-team='heroes']");
    await expect(scene).toBeVisible();
    await expect(scene).toHaveAttribute("data-animations", "disabled");
    ...
    const expectedNames = (["female", "male"] as const).flatMap((gender) => (
      [firstVariant, firstVariant + 1].map((variant) => (
        `pugilist-${gender}-${String(variant).padStart(2, "0")}-v1`
      ))
    ));
    expect(sources).toHaveLength(4);
    expect(sources.every(({ naturalWidth, naturalHeight }) => (
