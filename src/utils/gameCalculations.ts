...
export function equipItem(
  ...
  // Level requirement check (Proposition A)
...
export const generateSingleNoviceHero = (unlockedRaces: string[] = ["Humain"]): Hero => {
  ...
  const initialHeroStub: Omit<Hero, "calculatedStats"> = {
    ...
    name: heroName,
    gender: gender,
    spriteIndex: Math.floor(Math.random() * 20),
...
export const evaluateAutomaticClassChange = (
  ...
  // Option C: Orientation bonus based on Level
  // As a Novice reaches higher levels, they gain progressive score/determination for all jobs.
  const extraLevels = Math.max(0, hero.level - 10);
  const levelBonus = extraLevels * 4.0; // +4 points per level above 10
  ...
  for (const cond of tier1Classes) {
    ...
    // Apply Option C: level-based orientation score bonus
    score += levelBonus;
  ...
  // Option A & C: Softer decision threshold + level-based threshold easing
  // Baseline threshold is 40 (Option A). Decreases by 2 per level above 10 (Option C) to ensure they eventually promote.
  const minScoreThreshold = Math.max(25, 40 - extraLevels * 2);
  
  if (bestCandidate.score < minScoreThreshold) {
    return {
      newClass: null,
      reason: `Affinité trop incertaine pour une transition automatique (${bestCandidate.type} : ${bestCandidate.score.toFixed(1)}/seuil ${minScoreThreshold.toFixed(1)})`
    };
  }

  // Option A & C: Softer gap threshold that becomes even softer as level increases
  if (candidates.length > 1) {
    const secondBest = candidates[1];
    const scoreGap = bestCandidate.score - secondBest.score;
    
    // Baseline gap is 3.0 (Option A). Decreases by 0.5 per level above 10 (Option C) to resolve dilemmas.
    const gapThreshold = Math.max(0.5, 3.0 - extraLevels * 0.5);

    if (scoreGap < gapThreshold) {
      return {
        newClass: null,
        reason: `Dilemme d'orientation entre ${bestCandidate.type} (${bestCandidate.score.toFixed(1)}) et ${secondBest.type} (${secondBest.score.toFixed(1)}) (seuil d'écart: ${gapThreshold.toFixed(1)})`
      };
    }
  }

  const reason = extraLevels > 0 
    ? `Forte prédisposition facilitée par l'expérience accumulée (score d'affinité: ${bestCandidate.score.toFixed(1)} au niveau ${hero.level})`
    : `Forte prédisposition de profil (score d'affinité: ${bestCandidate.score.toFixed(1)})`;
...
export function applySplitDamageDefenseOrResistance(damageAfterCrit: number, damageTypes: DamageType[], monster: Monster): number {
  if (!damageTypes || damageTypes.length === 0) {
    return applyMonsterDefenseOrResistance(damageAfterCrit, "physical", monster);
  }
  if (damageTypes.length === 1) {
    return applyMonsterDefenseOrResistance(damageAfterCrit, damageTypes[0], monster);
  }
  const count = damageTypes.length;
  let accumulated = 0;
  for (const dType of damageTypes) {
    const partDamage = damageAfterCrit / count;
    accumulated += applyMonsterDefenseOrResistance(partDamage, dType, monster);
  }
  return Math.max(1, Math.round(accumulated));
}
