      ...
      {props.items.map(({ instanceId, itemTypeLabel, item }) => {
        ...
        return <Card key={instanceId} data-testid={`storage-item-${instanceId}`} selected={selected} className="flex flex-col">
          <button type="button" aria-pressed={selected} onClick={() => props.onSelect(instanceId)} className="min-h-11 flex-1 text-left focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#caa050]">
            <span className="flex items-start justify-between gap-2"><span className="min-w-0 break-words"><span className="block text-[9px] uppercase text-[#8f7a67]">{itemTypeLabel}</span><strong className="font-serif text-xs text-[#eadabc]">{item.name}</strong></span><span className="shrink-0 text-[9px] uppercase text-[#caa050]">{item.rarityLabel}</span></span>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 border-t border-[#3e2b1f] pt-2">
            {!pendingRecycle && <Button type="button" size="sm" variant={selected ? "secondary" : "primary"} className="min-w-0 whitespace-normal px-2 text-xs" onClick={() => props.onSelect(instanceId)}>{selected ? "Sélectionné" : "Équiper"}</Button>}
            {props.canRecycle && props.onRecycle && (pendingRecycle ? <><Button type="button" size="sm" variant="primary" disabled={!props.canMutate} className="min-w-0 whitespace-normal px-2 text-xs" onClick={() => { props.onRecycle?.(instanceId); setPendingRecycleId(null); }}>Oui</Button><Button type="button" size="sm" variant="danger" className="min-w-0 whitespace-normal px-2 text-xs" onClick={() => setPendingRecycleId(null)}>Non</Button></> : <Button type="button" size="sm" variant="danger" disabled={!props.canMutate} className="min-w-0 whitespace-normal px-2 text-xs" onClick={() => setPendingRecycleId(instanceId)}>Recycler</Button>)}
          </div>
        </Card>;
      })}
    </div>}
