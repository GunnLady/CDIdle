...
export default function DungeonProgressBanner(props: DungeonProgressBannerProps) {
  ...
  return (
    <aside aria-label="Progression du groupe dans le donjon" className="h-full rounded-xl border border-[#5c402b] bg-[#18100a] p-3 shadow-lg xl:rounded-none xl:border-0 xl:bg-transparent xl:py-0 xl:pr-0 xl:shadow-none">
      <div className="flex flex-wrap items-center justify-center gap-3 text-center md:text-left xl:h-full">
        ...
        <div className="flex w-full flex-nowrap items-center justify-center gap-2 md:w-auto">
          {!hasActiveHeroes ? (
            <Button type="button" size="sm" aria-label="Préparer le groupe" onClick={() => props.onNavigate("heroes")}><span className="sm:hidden">Groupe</span><span className="hidden sm:inline">Préparer le groupe</span></Button>
          ) : props.view.canToggleAutoExplore ? (
            <Button type="button" size="sm" onClick={props.onToggleAutoExplore}>
              {props.view.autoExplore ? <><Pause className="h-3 w-3" />Pause</> : <><Play className="h-3 w-3" />Reprendre</>}
            </Button>
          ) : null}
          <Button type="button" size="sm" variant="primary" aria-label="Voir le Donjon" onClick={() => props.onNavigate("dungeon")}><Shield className="h-3 w-3" /><span className="sm:hidden">Donjon</span><span className="hidden sm:inline">Voir le Donjon</span></Button>
