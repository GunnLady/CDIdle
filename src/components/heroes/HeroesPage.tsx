...
export default function HeroesPage(props: HeroesPageProps) {
  ...
  return <section aria-labelledby="heroes-page-title" className="space-y-5 animate-fade-in motion-reduce:animate-none">
    ...
    <div className="grid grid-cols-1 items-start gap-4 xl:h-[calc(100vh-15rem)] xl:min-h-[42rem] xl:max-h-[56rem] xl:grid-cols-[minmax(18rem,0.85fr)_minmax(24rem,1.15fr)_minmax(22rem,1fr)] xl:items-stretch">
      <div data-testid="heroes-left-column" className="space-y-4 xl:flex xl:min-h-0 xl:flex-col xl:space-y-0 xl:gap-4">
        <DungeonPartyManager party={view.party} floor={props.activeDungeonFloor} room={props.activeDungeonRoom} onSelectHero={setSelectedHeroId} onOpenDungeon={() => props.onGoToTab?.("dungeon")} />
