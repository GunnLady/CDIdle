...
export default function DungeonPartyPanel(props: {
  ...
  return <Panel title="Gestion du groupe" subtitle={`${props.party.filter(Boolean).length}/${props.party.length} héros engagés`} testId="dungeon-party-roster" variant="strong" className="h-full w-full" contentTestId="dungeon-party-scroll">
    <div className="grid grid-cols-2 gap-2">
      {props.party.map((hero, index) => hero ? <Card data-testid="dungeon-party-slot" key={hero.id} selected={props.selectedHeroId === hero.id} className={`flex h-[18rem] flex-col p-2 ${hero.isKnockedOut ? "opacity-60" : ""}`}>
        ...
        {hero.isKnockedOut && <p className="mt-2 rounded border border-red-700/60 bg-red-950/40 p-2 text-center text-[9px] font-bold uppercase text-red-200">KO — attend une salle de repos</p>}
