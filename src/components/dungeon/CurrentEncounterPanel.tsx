  ...
  return <Panel title="Rencontre actuelle" subtitle={props.view?.location ?? "Aucune rencontre engagée"} testId="dungeon-current-encounter" variant="strong" className="flex h-[675px] min-h-[675px] max-h-[675px] w-full flex-col overflow-hidden" contentClassName="flex min-h-0 flex-1 flex-col">
    <div className="flex min-h-0 flex-1 flex-col">
      {!props.view ? <EmptySlot className="flex-1 flex-col gap-3 py-8"><Compass className="h-8 w-8" /><p className="text-xs">L’escouade attend votre ordre.</p></EmptySlot> : <>
        ...
        {props.view.enemies.length > 0 && <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3" data-testid="dungeon-enemy-group">{props.view.enemies.map((enemy) => {
          const hasKnownHealth = enemy.hp !== null && enemy.maxHp !== null;
          const hpPercent = hasKnownHealth && enemy.maxHp > 0
            ? Math.max(0, Math.min(100, Math.round((enemy.hp / enemy.maxHp) * 100)))
            : 0;
          return <article key={enemy.id} className="rounded-lg border border-[#49311f] bg-[#110b06] p-3"><div className="flex items-start justify-between gap-2"><div><h5 className="text-xs font-bold text-[#e7d7bc]">{enemy.name}</h5><p className="mt-1 text-[9px] uppercase text-[#8f7a67]">{enemy.role ?? "ennemi"}</p></div><span className="text-[9px] text-amber-300">{enemy.intent}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-black/50"><div className="h-full bg-rose-600" style={{ width: `${hpPercent}%` }} /></div><p className="mt-1 text-[9px] text-[#b9a48b]">{hasKnownHealth ? `${enemy.hp}/${enemy.maxHp} PV` : "PV historiques inconnus"}</p>{enemy.effects.length > 0 && <p className="mt-1 text-[9px] text-sky-300">{enemy.effects.join(" · ")}</p>}</article>;
        })}</div>}        <div data-testid="dungeon-encounter-transcript" className="mt-3 min-h-0 flex-1 space-y-2 overflow-y-auto rounded-lg border border-[#3a281a] bg-[#0f0a07] p-3" aria-live="polite">
