import type { StoredForgeMaterialStack, StoredItemInstance } from "../types";
...
export function recycleItem(storedItems: StoredItemInstance[], forgeMaterials: StoredForgeMaterialStack[], forgeUnlocked: boolean, instanceId: string): { ok: true; storedItems: StoredItemInstance[]; forgeMaterials: StoredForgeMaterialStack[]; rewards: StoredForgeMaterialStack[] } | { ok: false; error: "FORGE_LOCKED" | "ITEM_NOT_FOUND" } {
  if (!forgeUnlocked) return { ok: false, error: "FORGE_LOCKED" };
  const result = scrapItemFromStorage(storedItems, forgeMaterials, instanceId);
