  ...
  useEffect(() => {
    if (!currentUser) {
      ...
      if (savedGame) {
        ...
        if (parsed.heroes) {
          const updatedHeroes = parsed.heroes.map((h: Hero) => {
            const spriteIndex = h.spriteIndex !== undefined ? h.spriteIndex : Math.floor(Math.random() * 20);
            return {
              ...h,
              spriteIndex
            };
          });
          dungeon.setHeroes(updatedHeroes);
        }
          ...
          if (docSnap.exists()) {
            ...
            if (parsed.heroes) {
              const refreshed = parsed.heroes.map((h: Hero) => {
                ...
                refreshedH.spriteIndex = h.spriteIndex !== undefined ? h.spriteIndex : Math.floor(Math.random() * 20);
                return refreshedH;
              });
          ...
          <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-xs flex items-center justify-center p-4 font-sans select-none">
            <div className="w-full max-w-sm bg-[#160f0a] border-2 border-[#d4af37] rounded-3xl p-6 shadow-[0_15px_45px_rgba(0,0,0,0.95)] relative overflow-hidden">
              ...
              <div className="text-center mb-5 flex flex-col items-center">
                <HeroPortrait hero={pendingRecruit} size="lg" noBorder={true} noBg={true} noPadding={true} className="mb-3 shadow-lg" />
