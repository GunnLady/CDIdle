...
export default function Panel({ title, subtitle, testId, className, contentClassName, contentTestId, variant = "default", titleAs: Title = "h3", children }: PanelProps) {
  ...
  const titleId = useId();
  return (
    <section aria-labelledby={titleId} data-testid={testId} className={classNames("min-w-0 rounded-ui-panel border-ui-border p-4 shadow-ui-panel", strong ? "border-2 bg-ui-panel-strong" : "border bg-ui-panel", className)}>
