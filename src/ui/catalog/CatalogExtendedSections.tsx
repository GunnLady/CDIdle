import { Database, Info, Search, Shield, Sparkles, Swords, UserRound } from "lucide-react";
...
export default function CatalogExtendedSections() {
  ...
  return <>
    <CatalogSection title="Contrôles" subtitle="Champs, sélections, cases et états" testId="catalog-control-variants">
    ...
    <CatalogSection title="Cartes, sélection et métriques" subtitle="États statiques et interactifs" testId="catalog-selection-metrics">
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <Card><strong className="text-sm">Épée de voyage</strong><p className="text-xs text-ui-text-muted">Objet disponible.</p></Card>
            <Card selected><strong className="text-sm">Bouclier ancien</strong><p className="text-xs text-ui-text-muted">Objet sélectionné.</p></Card>
          </div>
          ...
          <div className="flex items-center gap-3"><Tooltip label="Aide contextuelle" content="Information disponible au survol et au focus"><Info className="h-5 w-5" /></Tooltip><span className="text-sm">Aide contextuelle accessible</span></div>
        </div>
      </div>
    </CatalogSection>

    <CatalogSection title="Progressions" subtitle="Valeurs métier, jauges compactes et progression du Donjon" testId="catalog-progressions">
    ...
    <CatalogSection title="Navigation et bannières" subtitle="États persistants de l’application" testId="catalog-navigation-status">
    ...
    <CatalogSection title="États d’entrée et chargement" subtitle="Authentification, récupération et erreur bloquante" testId="catalog-entry-states">
      <div className="grid gap-4 lg:grid-cols-2">
    ...
    <CatalogSection title="Notifications dynamiques" subtitle="Annonce accessible et action flottante" testId="catalog-dynamic-feedback">
    ...
    <CatalogSection title="Dialogues" subtitle="Dialogue standard, choix et opération bloquée" testId="catalog-dialog-variants">
      <div className="flex flex-wrap gap-3"><Button onClick={() => setDialogExample("basic")}>Ouvrir le dialogue</Button><Button className="inline-flex items-center gap-2 whitespace-nowrap" onClick={() => setDialogExample("choice")}><Swords className="h-4 w-4 shrink-0" />Ouvrir un choix</Button><Button onClick={() => setDialogExample("blocking")}>Ouvrir une opération bloquée</Button></div>
    </CatalogSection>

    {dialogExample === "basic" && <Dialog title="Confirmer l'action" description="Exemple transversal sans règle métier." onDismiss={() => setDialogExample(null)} footer={<><Button onClick={() => setDialogExample(null)}>Annuler</Button><Button variant="primary" onClick={() => setDialogExample(null)}>Confirmer</Button></>}><p className="text-sm text-ui-text">Le focus reste dans cette fenêtre tant qu'elle est ouverte.</p></Dialog>}
