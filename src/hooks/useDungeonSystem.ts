...
import { BOSS_LOOT_TABLES_REGISTRY } from "../data/bossLootTables";
import {
...
interface EvolutionRewards {
  weaponId: string;
  accessoryId: string;
}

function getEvolutionRewardsForClass(classType: ClassType): EvolutionRewards | null {
  switch (classType) {
    case "Guerrier":
      return { weaponId: "basic_sword", accessoryId: "sturdy_travel_belt" };
    case "Voleur":
      return { weaponId: "basic_dagger", accessoryId: "knotted_leather_bracelet" };
    case "Archer":
      return { weaponId: "basic_longbow", accessoryId: "knotted_leather_bracelet" };
    case "Mage":
      return { weaponId: "basic_staff", accessoryId: "copper_focus_ring" };
    case "Acolyte":
      return { weaponId: "basic_mace", accessoryId: "silver_ring" };
    case "Aède":
      return { weaponId: "basic_lute", accessoryId: "silver_ring" };
    case "Druide":
      return { weaponId: "basic_bo", accessoryId: "sturdy_travel_belt" };
    case "Artificier":
      return { weaponId: "basic_rifle", accessoryId: "copper_focus_ring" };
    case "Pugiliste":
      return { weaponId: "basic_knuckles", accessoryId: "ashwood_bracelet" };
    default:
      return null;
  }
}
...
export function useDungeonSystem({
  ...
  const handleHeroClassEvolution = useCallback((hero: Hero, nextClass: ClassType): Hero => {
    const nextClassInfo = CLASS_INFO_LIST.find((c) => c.type === nextClass);
    if (!nextClassInfo) {
      return hero;
    }

    const rewards = getEvolutionRewardsForClass(nextClass);
    if (!rewards) {
      return hero;
    }

    const weaponItem = getItemById(rewards.weaponId);
    const accItem = getItemById(rewards.accessoryId);
    if (!weaponItem || !accItem) {
      return hero;
    }

    // Skill evolution:
    // Remove the current Novice active skills
    const noviceActiveSkills = ["heavy_blow", "guard_stance"];
    const activeSkills = (hero.activeSkills || []).filter(skillId => !noviceActiveSkills.includes(skillId));

    // Learn active skills from the selected T1 class (Mage/Acolyte learn 2, others learn 1)
    const isMageOrAcolyte = nextClass === "Mage" || nextClass === "Acolyte";
    const numSkillsToLearn = isMageOrAcolyte ? 2 : 1;

    const classActiveSkills = nextClassInfo.activeSkills || [];
    const learnedSkills: string[] = [];

    for (let i = 0; i < numSkillsToLearn; i++) {
      const skill = classActiveSkills.find(s => !activeSkills.includes(s) && !learnedSkills.includes(s));
      if (skill) {
    ...
    // Learn one passive skill from the selected T1 class
    const passiveSkills = [...(hero.passiveSkills || [])];
    const classPassiveSkills = nextClassInfo.passiveSkills || [];
    const passiveToLearn = classPassiveSkills.find(p => !passiveSkills.includes(p));
    if (passiveToLearn) {
      passiveSkills.push(passiveToLearn);
    }

    // Update hero properties
    const evolvedHero: Hero = {
      ...hero,
      classType: nextClass,
      activeSkills,
      passiveSkills
    ...
    const finalEvolvedHero = refreshHeroDerivedStats(evolvedHero);
    ...
    setStoredItems((prev) => {
      const updated = [...prev];
      addItemToStorage(updated, rewards.weaponId, "common", 1);
      addItemToStorage(updated, rewards.accessoryId, "common", 1);
      return updated;
    });

    // Logging the transition
    addLog(`🔮 VOCATION RÉVÉLÉE ! ${hero.name} s'est spécialisé en [${nextClassInfo.name}] !`, "victory");
    
    learnedSkills.forEach(skillId => {
      const skillInfo = getSkillById(skillId);
      const skillName = skillInfo ? skillInfo.name : skillId;
      addLog(`✨ Nouvelle compétence active : ${skillName}`, "victory");
    });

    if (passiveToLearn) {
      const passiveInfo = getSkillById(passiveToLearn);
      const passiveName = passiveInfo ? passiveInfo.name : passiveToLearn;
      addLog(`✨ Nouvelle compétence passive : ${passiveName}`, "victory");
    }

    addLog(`⚔️ Arme reçue : ${weaponItem.name} (Niveau 10)`, "loot");
    addLog(`📿 Accessoire reçu : ${accItem.name} (Niveau 10)`, "loot");

    return finalEvolvedHero;
  }, [setStoredItems, addLog]);
  ...
  const addXpToHero = useCallback((hero: Hero, xpEarned: number): Hero => {
    ...
    while (newXp >= needed) {
      ...
      // Random skill learning for Tier 1 heroes starting at level 13
      const classInfo = CLASS_INFO_LIST.find((c) => c.type === hero.classType);
      if (classInfo && classInfo.tier === 1 && newLvl >= 13) {
        const unlearnedActives = (classInfo.activeSkills || []).filter(s => !activeSkills.includes(s));
        const unlearnedPassives = (classInfo.passiveSkills || []).filter(s => !passiveSkills.includes(s));
        
        const candidates = [
          ...unlearnedActives.map(s => ({ type: "active" as const, id: s })),
          ...unlearnedPassives.map(s => ({ type: "passive" as const, id: s }))
        ];

        if (candidates.length > 0) {
          // Progressively increasing chance starting at 10% at level 13 and scaling exponentially to 75% at level 30 (capping the max reference level at 30)
          const progress = (Math.min(newLvl, 30) - 13) / (30 - 13);
          const chance = 0.10 * Math.pow(7.5, progress);

          if (Math.random() < chance) {
            const chosen = candidates[Math.floor(Math.random() * candidates.length)];
            if (chosen.type === "active") {
              activeSkills.push(chosen.id);
            } else {
              passiveSkills.push(chosen.id);
            }
            const skillInfo = getSkillById(chosen.id);
            const skillName = skillInfo ? skillInfo.name : chosen.id;
            const skillTypeLabel = chosen.type === "active" ? "active" : "passive";
            addLog(`🔮 ${hero.name} a appris une compétence ${skillTypeLabel} supplémentaire : [${skillName}] !`, "victory");
          }
        }
      }
    }
    ...
    if (leveledUp) {
      ...
      if (fullyUpgradedHero.classType === "Novice" && fullyUpgradedHero.level >= 10) {
        ...
        if (evaluation.newClass) {
          return handleHeroClassEvolution(fullyUpgradedHero, evaluation.newClass);
        } else {
  ...
  }, [addLog, buildings, handleHeroClassEvolution]);
  ...
  const runCombatTick = useCallback(() => {
    ...
    if (vicStep > 0) {
      ...
      if (vicStep === 1) {
        ...
        const lootTable = BOSS_LOOT_TABLES_REGISTRY[monster.name];
        ...
        // Gold: custom range if defined, else monster.goldYield
        let rawGold = monster.goldYield;
        if (lootTable && lootTable.goldRange) {
          const [minG, maxG] = lootTable.goldRange;
          rawGold = Math.floor(Math.random() * (maxG - minG + 1)) + minG;
        }
        const baseGoldLoot = Math.floor(rawGold * goldBonusMult * (1 + chefLvl * chefBonus));
        ...
        // Award forge materials and item rolls
        if (lootTable) {
          // 1. Roll materials before updating state to guarantee correct logging counts
          const materialsToGive: { materialId: string; rarity: Rarity; count: number; displayName: string }[] = [];
          lootTable.materials.forEach((matReward) => {
            if (Math.random() < matReward.chance) {
              const count = Math.floor(Math.random() * (matReward.maxCount - matReward.minCount + 1)) + matReward.minCount;
              if (count > 0) {
                materialsToGive.push({
                  materialId: matReward.materialId,
                  rarity: matReward.rarity,
                  count,
                  displayName: matReward.displayName
                });
              }
            }
          });

          // Update forge materials state
          if (materialsToGive.length > 0) {
            setForgeMaterials(prev => {
              const updated = prev.map(m => ({ ...m }));
              materialsToGive.forEach((g) => {
                const existing = updated.find(m => m.materialId === g.materialId && m.rarity === g.rarity);
                if (existing) {
                  existing.count += g.count;
                } else {
                  updated.push({ materialId: g.materialId, rarity: g.rarity, count: g.count });
                }
              });
              return updated;
            });
            ...
            // Post material logs
            materialsToGive.forEach((g) => {
              const rarityFrench = g.rarity === "common" ? "commun" : g.rarity === "uncommon" ? "atypique" : g.rarity === "rare" ? "rare" : g.rarity === "epic" ? "épique" : "légendaire";
              addLog(`⚙️ Matériau : +${g.count} [${g.displayName}] (${rarityFrench}) trouvé sur ${monster.name}.`, "loot");
            });
          }

          // Mention misses for materials that had a < 100% chance
          lootTable.materials.forEach((matReward) => {
            if (matReward.chance < 1.0) {
              const dropped = materialsToGive.some(m => m.materialId === matReward.materialId);
              if (!dropped) {
                const pct = Math.round(matReward.chance * 100);
                addLog(`💨 Pas de matériaux de type [${matReward.displayName}] supplémentaires sur ${monster.name} (${pct}% de chance).`, "info");
              }
            }
          });

          // 2. Roll item drops
          lootTable.items.forEach((itemReward) => {
            if (Math.random() < itemReward.chance) {
              let pool = NOVICE_BASIC_ITEM_LIST;
              if (itemReward.levelMin !== undefined || itemReward.levelMax !== undefined) {
                const minL = itemReward.levelMin !== undefined ? itemReward.levelMin : 0;
                const maxL = itemReward.levelMax !== undefined ? itemReward.levelMax : 100;
                pool = ITEM_LIBRARY.filter(item => {
                  const reqLvl = item.requiredLevel !== undefined ? item.requiredLevel : 1;
                  return reqLvl >= minL && reqLvl <= maxL;
                });
              }
              ...
              if (pool.length > 0) {
                ...
                setStoredItems(prev => {
                  const updated = [...prev];
                  addItemToStorage(updated, randomItem.id, itemReward.rarity, 1);
                  return updated;
                });
                const rarityFrench = itemReward.rarity === "common" ? "Commun" : itemReward.rarity === "uncommon" ? "Atypique" : itemReward.rarity === "rare" ? "Rare" : itemReward.rarity === "epic" ? "Épique" : "Légendaire";
                const logType = itemReward.rarity === "legendary" ? "victory" : "loot";
                addLog(`💎 Butin Boss : [${randomItem.name}] (${rarityFrench}) obtenu ! (${Math.round(itemReward.chance * 100)}% de chance)`, logType);
              }
            }
          });

          // 3. Roll blueprints
          lootTable.blueprints.forEach((bpReward) => {
            if (Math.random() < bpReward.chance) {
              let pool = ITEM_LIBRARY;
              if (bpReward.levelMin !== undefined || bpReward.levelMax !== undefined) {
                const minL = bpReward.levelMin !== undefined ? bpReward.levelMin : 0;
                const maxL = bpReward.levelMax !== undefined ? bpReward.levelMax : 100;
                pool = ITEM_LIBRARY.filter(item => {
                  const reqLvl = item.requiredLevel !== undefined ? item.requiredLevel : 1;
                  return reqLvl >= minL && reqLvl <= maxL;
                });
              ...
              if (pool.length > 0) {
                const randomItem = pool[Math.floor(Math.random() * pool.length)];
                setItemBlueprints(prev => {
                  const updated = [...prev];
                  const existingIndex = updated.findIndex(b => b.itemId === randomItem.id);
                  if (existingIndex !== -1) {
                    updated[existingIndex] = { ...updated[existingIndex], unlocked: true };
                  } else {
                    updated.push({ itemId: randomItem.id, unlocked: true });
                  }
                  return updated;
                });
                addLog(`📜 Plan de Forge : Plan de [${randomItem.name}] débloqué ! (${Math.round(bpReward.chance * 100)}% de chance)`, "loot");
              }
            }
          });

        } else {
          // Normal monster material drops (35% chance to drop)
          if (Math.random() < 0.35) {
    ...
    if (activeEncType && activeEncType !== "fight") {
      ...
      if (activeEncType === "treasure" || activeEncType === "rest") {
        ...
        if (step === 4) {
          ...
          const xpReward = Math.round(30 * (1 + (floor - 1) * 0.15));
          const eligibleHeroesCount = updatedHeroes.filter(h => h.isActive && h.currentHp > 0).length;
          updatedHeroes = updatedHeroes.map((hero) => {
            if (hero.isActive && hero.currentHp > 0) {
              const baseShare = eligibleHeroesCount > 0 ? (xpReward / eligibleHeroesCount) : 0;
      ...
      if (details) {
        ...
        if (bestSelection) {
          ...
          if (step === 4) {
            // 50% of the total XP is given to the solver, and 50% is shared with the active/alive team
            const xpReward = Math.round(40 * (1 + (floor - 1) * 0.15));
            const solverShareBase = Math.round(xpReward * 0.5);
            const teamSharePool = xpReward - solverShareBase;

            const eligibleHeroes = updatedHeroes.filter(h => h.isActive && h.currentHp > 0);
            const teamSharePerHero = eligibleHeroes.length > 0 ? Math.max(1, Math.round(teamSharePool / eligibleHeroes.length)) : 0;

            updatedHeroes = updatedHeroes.map((h) => {
              if (h.isActive && h.currentHp > 0) {
                const isSolver = h.id === bestHero.id;
                const humanBonus = h.race === "Humain" ? 1.10 : 1.0;
                const baseReward = (isSolver ? solverShareBase : 0) + teamSharePerHero;
                const finalXpEarned = Math.round(baseReward * humanBonus);
                if (finalXpEarned > 0) {
                  return addXpToHero(h, finalXpEarned);
                }
              }
              return h;
            });
    ...
    if (!monster) {
      ...
      const spawnedMonster: Monster = {
        ...
        xpYield: Math.floor(selectedRaw.xpYield * scaleMultiplier * 2),
      ...
      if (heroIndex !== -1) {
        ...
        if (hero.currentHp > 0) {
          ...
          if (!skillUsed) {
            ...
            const attackSpeed = weapon?.attackSpeed ?? 1.0;

            // Proposition 1: Double Frappe Agile (Modèle Multi-strike Cumulatif)
            // Cumulative percentage points = (attackSpeed - 1.0) * 100 + hero's speed substat
            const weaponSpeedBonus = (attackSpeed - 1.0) * 100;
            const speedSubstatBonus = calculatedStats.speed;
            const totalMultiStrikeChance = Math.max(0, weaponSpeedBonus + speedSubstatBonus);

            // Calculate actual number of strikes (minimum 1)
            let strikes = 1;
            let remainingChance = totalMultiStrikeChance;
            while (remainingChance > 0) {
              if (remainingChance >= 100) {
                strikes++;
                remainingChance -= 100;
              } else {
                if (Math.random() < (remainingChance / 100)) {
                  strikes++;
                }
                break;
              }
            }
            // Cap at 3 strikes to ensure gameplay balance and combat log readability
            strikes = Math.min(3, strikes);

            // Execute each strike sequentially
            for (let strike = 1; strike <= strikes; strike++) {
              ...
              const speedPrefix = strikes > 1 ? `⚡ [Frappe agile ${strike}/${strikes}] ` : "";

              if (isCrit) {
                addLog(`${speedPrefix}⚔️ [Coup critique 🍀] ${hero.name} assène un coup dévastateur à ${monster.name} pour ${finalDmg} dégâts.`, "combat-hero");
              } else {
                addLog(`${speedPrefix}🗡️ ${hero.name} attaque ${monster.name} pour ${finalDmg} dégâts.`, "combat-hero");
              }
            }
          }
        }
  ...
  const handleToggleHeroActive = useCallback((heroId: string) => {
    setHeroes((prev) => {
      ...
      return prev.map((h) => {
        if (h.id === heroId) {
          let nextStatus: "idle" | "exploring" | "resting" = "idle";
          if (nextActiveVal) {
            nextStatus = "exploring";
          } else {
            const needsHpRest = h.currentHp < h.calculatedStats.maxHp;
            const maxMana = h.calculatedStats.maxMana || 0;
            const needsManaRest = maxMana > 0 && h.currentMana < maxMana;
            if (needsHpRest || needsManaRest) {
              nextStatus = "resting";
            }
          }
          return { ...h, isActive: nextActiveVal, status: nextStatus };
        }
        return h;
      });
    });
