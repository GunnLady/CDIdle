alter table public.alpha_error_reports
  drop constraint alpha_error_reports_category_check,
  drop constraint alpha_error_reports_http_check;

alter table public.alpha_error_reports
  add constraint alpha_error_reports_category_check
    check (category in ('react', 'javascript', 'unhandledrejection', 'timeout', 'api_4xx', 'api_5xx')),
  add constraint alpha_error_reports_http_check
    check ((category = 'api_4xx' and http_status is not null and http_status between 400 and 499)
        or (category = 'api_5xx' and http_status is not null and http_status between 500 and 599)
        or (category not in ('api_4xx', 'api_5xx') and http_status is null and error_code is null));
...
begin
  if p_user_id is null
     ...
     or p_category not in ('react', 'javascript', 'unhandledrejection', 'timeout', 'api_4xx', 'api_5xx')
     ...
     or (p_category = 'api_4xx' and (p_http_status is null or p_http_status not between 400 and 499))
     or (p_category = 'api_5xx' and (p_http_status is null or p_http_status not between 500 and 599))
     or (p_category not in ('api_4xx', 'api_5xx') and (p_http_status is not null or p_error_code is not null))
