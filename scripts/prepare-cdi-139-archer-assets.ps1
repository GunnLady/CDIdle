...
function Build-Sheet {
  ...
  if ($Paths.Count -ne 10) {
    throw "Exactly ten normalized Archer sprites are required per gender."
  }
...
foreach ($gender in @("male", "female")) {
  ...
  for ($variant = 1; $variant -le 10; $variant++) {
    ...
    $inputPath = Join-Path $resolvedSourceRoot "validated-$gender-v1\archer-$gender-$suffix-v1.png"
    $referencePath = Join-Path $resolvedNeutralRoot "$gender\novice-$gender-$suffix-v1.png"
    $outputPath = Join-Path $resolvedOutputRoot "$gender\archer-$gender-$suffix-v1.png"
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
      throw "Missing validated Archer source: $inputPath"
    }
    if (-not (Test-Path -LiteralPath $referencePath -PathType Leaf)) {
      throw "Missing normalized Novice reference: $referencePath"
    }

    Write-Output "Preparing Archer $gender $suffix"
    & $prepareScript `
      -InputPath $inputPath `
  ...
  Build-Sheet `
    ...
    -AlphaPath (Join-Path $resolvedOutputRoot "archer-$gender-sheet-alpha-v1.png") `
    -LightPath (Join-Path $previewRoot "archer-$gender-sheet-light-v1.png") `
    -DarkPath (Join-Path $previewRoot "archer-$gender-sheet-dark-v1.png")
}

Write-Output "Prepared 20 normalized CDI-139 Archer sprites and six sheet artifacts at $resolvedOutputRoot"
