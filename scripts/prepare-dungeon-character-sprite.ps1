...
public static class DungeonCharacterSpritePreparation
{
    ...
    public static void Prepare(string inputPath, string referencePath, string outputPath)
    {
        using (var source = new Bitmap(inputPath))
        using (var reference = new Bitmap(referencePath))
        using (var extracted = ExtractChroma(source))
        {
            var sourceBounds = FindAlphaBounds(extracted, 32);
            var targetBounds = FindAlphaBounds(reference, 32);
...
[DungeonCharacterSpritePreparation]::Prepare($InputPath, $ReferencePath, $OutputPath)
