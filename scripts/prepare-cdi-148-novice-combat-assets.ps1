...
public static class Cdi148NoviceCombatAssetPreparation
{
    ...
    public static string[] Prepare(string sourceRoot, string neutralRoot, string outputRoot)
    {
        ...
        if (results.Count == 0)
            throw new InvalidOperationException("No validated CDI-148 combat-idle source was found.");
        ...
        string gender)
    {
        ...
        var pattern = "novice-" + gender + "-??-combat-idle-v1.png";
        ...
        foreach (var sourcePath in sourcePaths)
        {
            ...
            var prefix = "novice-" + gender + "-";
            var indexText = fileName.Substring(prefix.Length, 2);
            int index;
            if (!Int32.TryParse(indexText, out index) || index < 1 || index > 10)
                throw new InvalidOperationException("Invalid Novice identity index in " + sourcePath);

            var neutralPath = Path.Combine(
                neutralRoot,
                gender,
                String.Format("novice-{0}-{1:D2}-v1.png", gender, index));
            ...
            using (var neutral = new Bitmap(neutralPath))
            {
                    ...
                    neutralBounds))
                {
                    ...
                    ValidateOutput(normalized, outputPath, neutral.Width, neutral.Height, neutralBounds);
                    ...
                    SavePreview(
                        ...
                        Path.Combine(
                            previewDirectory,
                            String.Format("novice-{0}-{1:D2}-combat-idle-light-v1.png", gender, index)));
                    SavePreview(
                        normalized,
                        Color.FromArgb(255, 20, 24, 33),
                        Path.Combine(
                            previewDirectory,
                            String.Format("novice-{0}-{1:D2}-combat-idle-dark-v1.png", gender, index)));

                    yield return String.Format(
                        "{0}-{1:D2}|{2}x{3}|bounds={4},{5},{6},{7}|feet={8}|pivot={9:F1}|bytes={10}",
                        gender,
                        index,
                        normalized.Width,
                        normalized.Height,
        ...
        Rectangle referenceBounds)
    {
        ...
        if (width > frameWidth || height > frameHeight)
            throw new InvalidOperationException(String.Format(
                "The normalized combat sprite would overflow its {0}x{1} frame: {2}x{3}.",
                frameWidth,
                frameHeight,
                width,
                height));
        ...
        Rectangle referenceBounds)
    {
        if (image.Width != expectedWidth || image.Height != expectedHeight)
            throw new InvalidOperationException("Output dimensions do not match the neutral reference: " + path);
        ...
        if (bounds.Bottom != referenceBounds.Bottom)
            throw new InvalidOperationException("Combat and neutral feet baselines differ: " + path);
        var referencePivot = image.Width / 2.0;
        var outputPivot = bounds.Left + bounds.Width / 2.0;
        if (Math.Abs(referencePivot - outputPivot) > 1.0)
            throw new InvalidOperationException("Combat and neutral horizontal pivots differ: " + path);
...
if (-not ("Cdi148NoviceCombatAssetPreparation" -as [type])) {
...
[Cdi148NoviceCombatAssetPreparation]::Prepare(
  $resolvedSourceRoot,
  $resolvedNeutralRoot,
  $resolvedOutputRoot
)
