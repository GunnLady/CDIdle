...
function Build-Sheet {
  ...
  if ($Paths.Count -ne 10) {
    throw "Exactly ten normalized Warrior sprites are required per gender."
  }
...
foreach ($gender in @("male", "female")) {
  ...
  for ($variant = 1; $variant -le 10; $variant++) {
    ...
    $inputPath = Join-Path $resolvedSourceRoot "validated-$gender-v1\warrior-$gender-$suffix-v1.png"
    $referencePath = Join-Path $resolvedNeutralRoot "$gender\novice-$gender-$suffix-v1.png"
    $outputPath = Join-Path $resolvedOutputRoot "$gender\warrior-$gender-$suffix-v1.png"
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
      throw "Missing validated Warrior source: $inputPath"
    }
  ...
  Build-Sheet `
    ...
    -AlphaPath (Join-Path $resolvedOutputRoot "warrior-$gender-sheet-alpha-v1.png") `
    -LightPath (Join-Path $previewRoot "warrior-$gender-sheet-light-v1.png") `
    -DarkPath (Join-Path $previewRoot "warrior-$gender-sheet-dark-v1.png")
}

Write-Output "Prepared 20 normalized CDI-137 Warrior sprites and six sheet artifacts at $resolvedOutputRoot"
