param(
  ...
  [Parameter(Mandatory = $true)] [string]$MageRoot,
...
function Build-Sheet {
  ...
  if ($Paths.Count -ne 10) {
    throw "Exactly ten normalized Artificer sprites are required per gender."
  }
      ...
      for ($index = 0; $index -lt $Paths.Count; $index++) {
        ...
        try {
          if ($sprite.Width -ne $cellWidth -or $sprite.Height -ne $cellHeight) {
            throw "Normalized Artificer sprites must share one canvas size."
          }
...
$resolvedMageRoot = (Resolve-Path -LiteralPath $MageRoot).Path
...
foreach ($gender in @("male", "female")) {
  ...
  for ($variant = 1; $variant -le 10; $variant++) {
    ...
    $referenceVariant = if ($gender -eq "female" -and $variant -eq 4) { "08" } else { "06" }
    $referencePath = Join-Path $resolvedMageRoot "$gender\mage-$gender-$referenceVariant-v1.png"
    $inputPath = Join-Path $resolvedSourceRoot "validated-$gender-v1\artificer-$gender-$suffix-v1.png"
    $outputPath = Join-Path $resolvedOutputRoot "$gender\artificer-$gender-$suffix-v1.png"
    if (-not (Test-Path -LiteralPath $referencePath -PathType Leaf)) {
      throw "Missing normalized Mage reference: $referencePath"
    }
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
      throw "Missing validated Artificer source: $inputPath"
    }

    Write-Output "Preparing Artificer $gender $suffix"
    & $prepareScript `
      -InputPath $inputPath `
  ...
  Build-Sheet `
    ...
    -AlphaPath (Join-Path $resolvedOutputRoot "artificer-$gender-sheet-alpha-v1.png") `
    -LightPath (Join-Path $previewRoot "artificer-$gender-sheet-light-v1.png") `
    -DarkPath (Join-Path $previewRoot "artificer-$gender-sheet-dark-v1.png")
}

Write-Output "Prepared 20 normalized CDI-144 Artificer sprites and six sheet artifacts at $resolvedOutputRoot"
