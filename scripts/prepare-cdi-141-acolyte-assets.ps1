...
function Build-Sheet {
  ...
  if ($Paths.Count -ne 10) {
    throw "Exactly ten normalized Acolyte sprites are required per gender."
  }
      ...
      for ($index = 0; $index -lt $Paths.Count; $index++) {
        ...
        try {
          if ($sprite.Width -ne $cellWidth -or $sprite.Height -ne $cellHeight) {
            throw "Normalized Acolyte sprites must share one canvas size."
          }
...
foreach ($gender in @("male", "female")) {
  ...
  for ($variant = 1; $variant -le 10; $variant++) {
    ...
    $inputPath = Join-Path $resolvedSourceRoot "validated-$gender-v1\acolyte-$gender-$suffix-v1.png"
    $referencePath = Join-Path $resolvedNeutralRoot "$gender\novice-$gender-$suffix-v1.png"
    $outputPath = Join-Path $resolvedOutputRoot "$gender\acolyte-$gender-$suffix-v1.png"
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
      throw "Missing validated Acolyte source: $inputPath"
    }
    if (-not (Test-Path -LiteralPath $referencePath -PathType Leaf)) {
      throw "Missing normalized Novice reference: $referencePath"
    }

    Write-Output "Preparing Acolyte $gender $suffix"
    & $prepareScript `
      -InputPath $inputPath `
  ...
  Build-Sheet `
    ...
    -AlphaPath (Join-Path $resolvedOutputRoot "acolyte-$gender-sheet-alpha-v1.png") `
    -LightPath (Join-Path $previewRoot "acolyte-$gender-sheet-light-v1.png") `
    -DarkPath (Join-Path $previewRoot "acolyte-$gender-sheet-dark-v1.png")
}

Write-Output "Prepared 20 normalized CDI-141 Acolyte sprites and six sheet artifacts at $resolvedOutputRoot"
