param([string]$SourceRoot = 'assets/design/hero-sprites/cdi-152', [string]$OutputRoot = 'assets/design/hero-sprites/cdi-152/normalized-alpha-v1')
...
public static class MageCombatNormalizer
{
...
foreach ($gender in @('male','female')) {
    foreach ($index in 1..10) {
        $name = 'mage-{0}-{1:D2}-combat-idle-v1.png' -f $gender,$index
        $inputPath = if ($gender -eq 'male' -and $index -eq 5) {
            Join-Path $SourceRoot ('alpha-source-v1/{0}' -f $name)
        } else {
            Join-Path $SourceRoot ('validated-{0}-v1/{1}' -f $gender,$name)
        }
        $outputPath = Join-Path $OutputRoot ('{0}/{1}' -f $gender,$name)
        [MageCombatNormalizer]::Normalize((Resolve-Path $inputPath).Path, [IO.Path]::GetFullPath($outputPath))
    }
}
