...
function Build-Sheet {
  ...
  if ($Paths.Count -ne 10) {
    throw "Exactly ten normalized Rogue sprites are required per gender."
  }
...
foreach ($gender in @("male", "female")) {
  ...
  for ($variant = 1; $variant -le 10; $variant++) {
    ...
    $inputPath = Join-Path $resolvedSourceRoot "validated-$gender-v1\rogue-$gender-$suffix-v1.png"
    $referencePath = Join-Path $resolvedNeutralRoot "$gender\novice-$gender-$suffix-v1.png"
    $outputPath = Join-Path $resolvedOutputRoot "$gender\rogue-$gender-$suffix-v1.png"
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
      throw "Missing validated Rogue source: $inputPath"
    }
    ...
    try {
      ...
      Write-Output "Preparing Rogue $gender $suffix"
      & $prepareScript `
  ...
  Build-Sheet `
    ...
    -AlphaPath (Join-Path $resolvedOutputRoot "rogue-$gender-sheet-alpha-v1.png") `
    -LightPath (Join-Path $previewRoot "rogue-$gender-sheet-light-v1.png") `
    -DarkPath (Join-Path $previewRoot "rogue-$gender-sheet-dark-v1.png")
}

Write-Output "Prepared 20 normalized CDI-138 Rogue sprites and six sheet artifacts at $resolvedOutputRoot"
