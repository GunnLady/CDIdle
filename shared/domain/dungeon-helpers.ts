import { Hero, DungeonEncounterType, Rarity, SkillInfo } from "../contracts/game.ts";
import { SKILLS_LIBRARY } from "../data/game-data.ts";
...
export function rollEncounterForgeMaterial(floor: number, rng: Rng): { materialId: string; rarity: Rarity; count: number; name: string } {
  const rand = rng.next();
