...
- [x] Il n'existe plus qu'une occurrence `case "guilde"` dans `getBuildingMaxLevel`.
- [x] Le Campement retourne 5 et les autres familles gardent leur valeur actuelle.
- [x] Un test de regression cible echouerait si la limite repassait a 1.
- [x] Aucun autre calcul de batiment n'est modifie.
...
Fournir le test rouge/vert, le diff exact du switch, les commandes executees et la confirmation qu'aucun identifiant persiste n'a change. Resultat : test de regression ajoute, check/audit/git diff propres ; CI a valider apres push.
