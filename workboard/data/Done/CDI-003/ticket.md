...
- [x] `npm run check` reproduit localement les controles principaux de la CI.
- [x] Le workflow s'execute sur PR vers `staging` et `main` ainsi que sur push vers ces branches.
- [x] ESLint inspecte les fichiers TypeScript/TSX sans reformater le depot.
- [x] Les permissions du workflow sont en lecture seule.
- [x] Le build produit toujours l'application actuelle.
...
Fournir les scripts finaux, la version Node, les controles executes, le lien du run CI et toute dette de lint volontairement conservee. Resultat local : check et audit production OK ; ESLint signale 133 avertissements historiques, dont le doublon guilde reserve a CDI-005.
